...
/// LivePay Payments API Service
class LivePaymentService {
  static final LivePaymentService _instance = LivePaymentService._internal();

  factory LivePaymentService() => _instance;

  LivePaymentService._internal();

  // Use your deployed Firebase Cloud Function endpoint
  static const String _cloudFunctionUrl =
      'https://us-central1-truehome-9a244.cloudfunctions.net/livePayment';
  static const String _statusUrl =
      'https://us-central1-truehome-9a244.cloudfunctions.net/livePaymentStatus';
  static const String _callbackUrl =
      'https://us-central1-truehome-9a244.cloudfunctions.net/livePaymentWebhook';
  ...
  Future<PaymentInitResponse> initiatePayment({
    ...
    try {
      ...
      debugPrint('═══════════════════════════════════════');
      debugPrint('🔵 LIVEPAY PAYMENTS: Initiating Transaction');
      debugPrint('═══════════════════════════════════════');
      ...
      if (userMessage.contains('Failed host lookup')) {
        userMessage =
            'Cannot reach LivePay servers. Check your internet connection or try using a VPN.';
        ...
        userMessage =
            'Request timed out. LivePay servers may be slow. Try again.';
      } else if (userMessage.contains('401') ||
          userMessage.contains('Unauthorized')) {
        userMessage = 'API key invalid. Check your LivePay credentials.';
  ...
  Future<bool> cancelPayment({required String transactionRef}) async {
    try {
      debugPrint('🔵 LIVEPAY PAYMENTS: Cancelling Transaction');
      debugPrint('Reference: $transactionRef\n');
...
class PaymentInitResponse {
  ...
  final String transactionReference; // LivePay's transaction reference
...
class PaymentStatusResponse {
  ...
  final String message;
  final double amount;
  final double transactionCharge; // LivePay's fee
